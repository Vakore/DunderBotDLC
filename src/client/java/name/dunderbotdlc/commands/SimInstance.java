          ...
          for (cursor.x = Math.floor(queryBB.minX); cursor.x <= Math.floor(queryBB.maxX); cursor.x++) {
              ...
              if (blockState != null) {
                  ...
                  if (isWater || isWaterlogged) {
                      double waterLevel = y + 1 - blockState.getFluidState().getHeight(world, blockPos);
                      if (queryBB.maxY >= waterLevel) {
                          waterBBs.add(new AABB((int)Math.floor(cursor.x), (int)Math.floor(cursor.y), (int)Math.floor(cursor.z), blockPos.getX() + 1, blockPos.getY() + waterLevel, blockPos.getZ() + 1));//cursor.toImmutable());
                      }
                  }
              }
    ...
    public void moveEntityWithHeading (double strafe, double forward) {
      ...
      if (this.isInWater || this.isInLava) {
        ...
        if (false/*isOnLadder(world, pos)*/) {
          this.velX = clamp(-physicsladderMaxSpeed, this.velX, physicsladderMaxSpeed);
          this.velZ = clamp(-physicsladderMaxSpeed, this.velZ, physicsladderMaxSpeed);
...
boolean isInWaterApplyCurrent (ClientWorld world, AABB bb, Vec3d vel) {
    ...
    ArrayList<AABB> waterBlocks = getWaterInBB(getPlayerBB(new Vec3d(this.x, this.y, this.z)).offset(0.0, 0.0, 0.0));
    ...
    for (AABB pos : waterBlocks) {
      ...
      Vec3d flow = world.getFluidState(block).getVelocity(world, block);//getFlow(world, block);
